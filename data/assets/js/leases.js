...
feedback.then(leases => {
    if(leases.length == 0)
    {
        ...
        leases.forEach(lease => {
            ...
            if(is_admin && lease.status == "PENDING APPROVAL") row_content += `<a onclick="changeStatus('authorize', ${lease.id})" class="btn btn-primary">Authorize</a></div>`;
            else if(is_admin && lease.status == "LEASED") row_content += ` <a onclick="changeStatus('end', ${lease.id})" class="btn btn-danger">End lease</a></div>`;
